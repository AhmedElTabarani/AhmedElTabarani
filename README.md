<div dir = rtl>

<span dir = rtl align = "center">
  
# السلام عليكم ورحمة الله وبركاته 
# مرحبًا بالجميع، أُدعى أحمد الطبراني
</span>

## من أنا؟
<div dir = ltr>
  
```js
Database db;
Person me = db.getInformationAbout("Ahmed El-Tabarani");
cout << me.ToString();
```
#### Output:
```swift
Name: Ahmed Moustafa Kamal El-Deen El-Tabarani
Age : 20
University : Suez Canal University
Faculty : Faculty of Computer and Information
```
</div>

## مهاراتي
<div dir = ltr>
  
```js
Database db;
Person me = db.getSkillsAbout("Ahmed El-Tabarani");
cout << me.ToString();
```
#### Output:
```swift
Languages:   ["C/C++", "php", "Javascript"],
Skills:      ["ProblemSolving", "DataStructures", "Algorithms", "Database"],
Frameworks : ["NodeJs", "Gdscript"],
Tools :      ["VsCode", "Git/Github", "Godot"]
```
</div>
<div dir = ltr>
  
</div>

## احصائياتي

<a href="https://github.com/AhmedElTabarani">
  <img height="180em" style="float: left" src="https://github-readme-stats-eight-theta.vercel.app/api/top-langs/?username=AhmedElTabarani&theme=react&layout=compact"/>
  <img height="180em" style="float: right" src="https://github-readme-stats.vercel.app/api?username=AhmedElTabarani&show_icons=true&theme=react&include_all_commits=true"/>
</a>

---

```js
Quran quran("An-Nahl", 97);
cout << quran.text("Arabic") <<'\n'<< quran.text("English");
```
#### Output:
<div align = "center">

```
مَنْ عَمِلَ صَالِحًا مِنْ ذَكَرٍ أَوْ أُنْثَىٰ وَهُوَ مُؤْمِنٌ فَلَنُحْيِيَنَّهُ حَيَاةً طَيِّبَةً ۖ وَلَنَجْزِيَنَّهُمْ أَجْرَهُمْ بِأَحْسَنِ مَا كَانُوا يَعْمَلُونَ

Whoever does good, whether male or female, and is a believer, We will surely bless them with a good life,  
and We will certainly reward them according to the best of their deeds.
```
</div>

</div>
